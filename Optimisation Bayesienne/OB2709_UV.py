...
############################!!!!!!!!!!!!!!!!!!!!!!##################################
#Modifier SCENARCHI = 3 dans resacartparm.py
#Scénario ayant comme entrée ["SSH"27,"SST"09] et en sortie ["SSH"09,"U"09,"V"09]
####################################################################################
...
from tensorflow.nn import swish, relu
from math import ceil

#THE ARCHITECTURE TO OPTIMIZE

def my_model(nc,nl, function):
  ArchiOut = []
  upfactor = 3
  factiv= relu
  ...
  ArchiB   = UpSampling2D((upfactor, upfactor), interpolation='bilinear')(ISSHreso)
  #ArchiB   = concatenate([ArchiB, ISSTR27], axis=1) # CHANNEL FIRST
  ISSTR09 = all_Kinput_img[1]
  ...
  for i in np.arange(5) : #7
    ...
    ArchiB = Conv2D(32,(3,3), activation=factiv, padding='same',kernel_initializer=init)(ArchiB)
    ArchiB = Conv2D(32,(3,3), activation=factiv, padding='same',kernel_initializer=init)(ArchiB)
    ArchiB = BatchNormalization(axis=3)(ArchiB)
  ArchiB = Conv2D(8,(3,3), activation=factiv, padding='same',kernel_initializer=init)(ArchiB)
  ArchiB = Conv2D(1,(1,1), activation=factout, padding='same',kernel_initializer=init,name='ArchiB')(ArchiB)
  ...
  if IS_SSTR09 : # SST R09 utilisï¿½ pour trouver U R09 et V R09
    ...
    ArchiUV  = concatenate([ArchiB, ISSTR09], axis=3) # CHANNEL LAST
...
#Function with all parameters to optimize in argument
def fit_with(nc, nl, function, lr):

  model = my_model(nc,  nl,function)
...
pbounds = {'nc': (1, 5), 'nl': (2,32) ,'function' :(0,3) ,  'lr': (-4.5,-1)} #function 0-1: linear, 1-2: sig01, 2-3: sigmoid
