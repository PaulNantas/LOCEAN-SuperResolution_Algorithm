# -*- coding: utf-8 -*-
###################### ARCHITECTURE PYTORCH 1.9 AVEC PREDICTIONS MOYENNE ET ECART TYPE #####################
...
Niter, Bsize = 50, 32
lr = 0.0005#2*(10**(-2.89637961)) (Learning rate obtenue par optimisation bayesienne) #learning rate utiisé dans l'architecture
...
#Il faut réaliser les calculs sur le GPU
...
#On définit chaque couche dans le init
  def __init__(self):
    ...
    self.conv_inputR2709 = nn.Conv2d(in_channels=2,out_channels=23,kernel_size=(3,3), padding='same',padding_mode='replicate')
    ...
    self.conv_23x23 = nn.Conv2d(23,23,kernel_size=(3,3), padding='same',padding_mode='replicate')
    ...
    self.conv_23x8 = nn.Conv2d(23,8, kernel_size=(3,3), padding='same', padding_mode='replicate')
...
#On définit l'architecture dans le forward
  def forward(self, x):
    #y = self.Up1(x[0]); 
    factiv = nn.LeakyReLU()
    ...
    out = self.conv_8x2(out); nn.ReLU(out);

    return out

#Chargement des données classique mais il faut les passer en torch.Tensor
def toTensor(x):
  X = []
  for idx in range(len(x)):
    shape = (x[idx].shape[0],1,x[idx].shape[1],x[idx].shape[2]) #Channel first
    X_int = torch.Tensor([i for i in x[idx]]).view(shape).to(dev) #Placer les variables sur le GPU
...
x_testPT = toTensor(x_test)
y_testPT = toTensor(y_test)

#Définition de la fontion de coçut
class custom_loss(torch.nn.Module):
  ...
  def forward(self,x,y, mode):
    ...
    shape = pred.shape[0]
    std = x[:,1].reshape(-1)
    std = std +   1*(10**(-8))*torch.ones(std.shape,device=device)#.to(dev)
    ...
    return (1/shape)*(torch.sum(loss1)+torch.sum(loss2))

#Entrainement du model
def train_model(model, x_train,y_train,x_valid,y_valid,  EPOCHS=Niter, BATCH_SIZE=Bsize,mode='custom'): #mode='MSE' ou 'custom' (type de loss)
  ...
  best_loss = 0 #Conserver la meilleur configuration du réseau avec la plus basse loss

  for epoch in range(EPOCHS):
    t0 = time.time() #Mesure le temps d'une epoch
    print(f"Epoch n° : {epoch} commencée")

    loss_train = 0
    loss_valid = 0

    for i in tqdm(range(0, len(x_train), BATCH_SIZE)): #Boucle de batch

      batch_X = []
      batch_y = []

      for k in range (len(x_train)): #Boucles pour que les batchs soient de la bonne forme
      ...
      loss = criterion(outputs,batch_y, mode)
      loss.backward()
      optimizer.step() #Entrainement du model 

      loss_train += loss
      print("\n","loss par Batch train=",loss,"\n")
    
    model.eval()     # Optional when not using Model Specific layer
    for i in tqdm(range(0, len(x_valid), 6)): #Boucle de validation 
      ...
      loss = criterion(target,batchv_y,mode)
      loss_valid += loss
      print("\n","loss par Batch valid=",loss,"\n")

    t1 = time.time()
    print("Temps d'une epoch : ", t1 - t0)
    ...
    if best_loss > loss_valid.item() or epoch ==0: #Enregistrement de la meileure configuration
      checkpoint = {
                'epoch': epoch,
                'model_state': model.state_dict(),
                'optimizer_state': optimizer.state_dict(),
            }
      torch.save(checkpoint ,os.path.join(dir_model,f'Trained_model-E{Niter}-BS{Bsize}_1.pt'))
...
model = Resac()
#Initialisation des poids avec kaiming_normal (équivalent de he_normal sous keras)
def weights_init(m):
...
torch.save(trained_model,os.path.join(dir_model,f'Trained_model-E{Niter}-BS{Bsize}_1.pt'))

print("ETAPE N°1 TERMINE","\n", "ENTRAINEMENT AVEC CUSTOM LOSS")


#model = torch.load(os.path.join(dir_model,f'Trained_model-E{Niter}-BS{Bsize}.pt'))
'''trained_model2, loss_list_train, loss_list_valid = train_model(trained_model, x_trainPT, y_trainPT, x_validPT, y_validPT, EPOCHS=Niter,BATCH_SIZE=Bsize)

torch.save(trained_model2,os.path.join(dir_model,f'Trained_model-E{Niter}-BS{Bsize}_2.pt'))
loss_list_train_2 = np.array(loss_list_train)
loss_list_valid_2 = np.array(loss_list_valid)

print("loss train", loss_list_train_2,"\n")
print("loss valid", loss_list_valid_2,"\n")
'''
loaded_checkpoint = torch.load(os.path.join(dir_model,f'Trained_model-E{Niter}-BS{Bsize}_1.pt'))
trained_model = Resac()
trained_model.load_state_dict(loaded_checkpoint['model_state'])
trained_model.eval() #Ligne importante si l'on recharge la structure de l'architecture
...
#plt.plot(epochs,loss_list_train_2, color='g', label='Training loss 2')
#plt.plot(epochs,loss_list_valid_2, color='orange', label='Valid loss 2')
plt.yscale("log")
...
pred = predict(trained_model, x_testPT).cpu().numpy()
#print("Shape", pred.shape)#(55,2,144,153)

jour = 30
SSH = pred[:,0,:,:]
STD = pred[:,1,:,:]

SSH_dec = decodage_sortie(SSH, parametre)
SSH_reel = decodage_sortie(y_test[0], parametre)
print("SHAPE1 :", STD.shape)
print("SHAPE2 :", SSH_dec.shape)


plt.imshow(SSH_dec[jour], origin='lower')
plt.colorbar()
plt.title("SSH R09 pred")
plt.savefig(os.path.join(dir_image, "SSH R09 pred"))
plt.clf()


plt.imshow(STD[jour], origin='lower')
plt.colorbar()
plt.title("SSH R09 STD")
plt.savefig(os.path.join(dir_image, "SSH R09 STD"))
plt.show()


plt.imshow(SSH_reel[jour], origin='lower')
plt.colorbar()
plt.title("SSH R09 réelle")
plt.savefig(os.path.join(dir_image, "SSH R09 réelle"))
plt.clf()
t1 = time.time()
temps = t1-t0
if temps//3600==0:
  print(f"Temps de l'apprentissage :{temps//60}min{temps%60}s" )
else :
  print(f"Temps de l'apprentissage : {int(temps//3600)}h{int((temps%3600)//60)}min{round(temps%60,2)}s" )
