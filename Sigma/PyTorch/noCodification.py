...
################################# ARCHITECTURE CLASSIQUE DE RESAC EN PYTORCH DE R81 A R09 SANS CODIFICATION DES DONNEES ######################"

Niter, Bsize = 30, 32
lr = 0.05#2*(10**(-2.89637961))
...
for i in np.arange(NvarOut):
    ...
    y_valid[i] = y_valid[i] - np.mean(y_valid[i])
...
#======================================================================
def sig01(x):
  return 2.1*torch.sigmoid(x)

class Sig01(nn.Module):
  def __init__(self):
     super().__init__()
...
class ResacR09(nn.Module):
  def __init__(self):
    ...
    self.conv_inputR2709 = nn.Conv2d(in_channels=2,out_channels=16,kernel_size=(3,3), padding=1)#,padding_mode='replicate')

    #Couches différentes à chaque résolution
    self.conv_32x32 = nn.Conv2d(32,32,kernel_size=(3,3), padding=1)#,padding_mode='replicate')
    self.conv_23x23 = nn.Conv2d(16,16,kernel_size=(3,3), padding=1)#,padding_mode='replicate')
    ...
    self.conv_23x8 = nn.Conv2d(16,8, kernel_size=(3,3), padding=1)#, padding_mode='replicate')
    self.conv_8x1 = nn.Conv2d(8,1, kernel_size=(3,3), padding=1)#, padding_mode='replicate')
    self.conv_8x2 = nn.Conv2d(8,2, kernel_size=(1,1), padding=1)#, padding_mode='replicate')


  def forward(self, x):
    #y = self.Up1(x[0]); 
    factiv = nn.LeakyReLU() #F.silu() #Sig01() # 
...
def train_model(model, x_train,y_train,x_valid,y_valid,  EPOCHS=Niter, BATCH_SIZE=Bsize):
  ...
  for epoch in range(EPOCHS):
    ...
    if best_loss > loss_valid.item() or epoch ==0: 
      checkpoint = {
                'epoch': epoch,
...
plt.title(f"Loss function avec {Niter} EPOCHS et {Bsize} Batch Size. Learning rate : {lr}")
...
def decodage_sortie(predictions,parametre):
  ...
  SSH_R27_dec = decodage(predictions, parametre[1][0])
...
def reg(a,b,x):
    return a*x + b 
b, a , R2= linr2(SSH_dec_flatten, SSH_true_flatten) #Regression définie dans resacartdef.py
...
print(pred.shape)
shap = pred.shape[2]
plt.imshow(pred[jour].reshape(shap,-1), origin='lower')
plt.colorbar()
...
if temps//3600==0:
  ...
  print(f"Temps de l'apprentissage : {int(temps//3600)}h{int((temps%3600)//60)}min{int((temps%3600)%60)}s" )
