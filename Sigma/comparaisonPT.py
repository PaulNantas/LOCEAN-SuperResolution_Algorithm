...
def train_model(model, x_train,y_train,x_valid,y_valid,  EPOCHS=Niter, BATCH_SIZE=Bsize,mode='RMSE'):
  loss_list_train = []
  loss_list_valid = []
...
trained_model, loss_list_train, loss_list_valid = train_model(model, x_trainPT, y_trainPT, x_validPT, y_validPT,  EPOCHS=Niter,BATCH_SIZE=Bsize, mode='MSE')
...
def decodage_sortie(predictions,parametre):
  ...
  SSH_R09_dec = decodage(predictions, parametre[1][1])
...
SSH_true = decodage_sortie(y_trainPT[-1],parametre).cpu().numpy()
...
plt.clf()
jour = 5
#plt.imshow(SSH_dec[jour].reshape(144,-1), origin='lower')
#plt.savefig(os.path.join(dir_image, "Prediction"))
#plt.clf()
plt.imshow(SSH_true[jour].reshape(144,-1), origin="lower")
